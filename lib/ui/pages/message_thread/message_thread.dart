...
import 'package:any1/ui/widgets/message_thread/receiver_message.dart';
import 'package:any1/ui/widgets/message_thread/sender_message.dart';
...
class MessageThread extends StatefulWidget {
  ...
  MessageThread(this.receiver, this.me, this.messageBloc, this.chatsCubit,
      this.typingNotificationBloc, {String chatId = ''})
      : this.chatId = chatId;
...
class _MessageThreadState extends State<MessageThread> {
  ...
  void initState() {
    ...
    widget.typingNotificationBloc.add(
      TypingNotificationEvent.onSubscribed(widget.me,
          usersWithChat: [widget.receiver.id!]),
    );
  }
                ...
                child: BlocBuilder<TypingNotificationBloc,
                  ...
                  builder: (_, state) {
                    ...
                    if (state is TypingNotificationReceivedSuccess &&
                        ...
                        state.event.from == widget.receiver.id) {
                      typing = true;
                    }
                    return HeaderStatus(
  ...
  _buildListOfMessages() => ListView.builder(
    ...
    itemBuilder: (__, idx) {
      if (messages[idx].message.from == widget.receiver.id) {
        _sendReceipt(messages[idx]);
        return Padding(
            padding: const EdgeInsets.only(bottom: 8.0),
            child: ReceiverMessage(messages[idx], widget.receiver.photoUrl));
      } else {
        return Padding(
            padding: const EdgeInsets.only(bottom: 8.0),
  ...
  void _updateOnMessageReceived() {
    ...
    _subscription = widget.messageBloc.stream.listen((state) async {
      ...
      if (state is MessageSentSuccess) {
        await messageThreadCubit.viewModel.sendMessage(state.message);
      }
  ...
  void _dispatchTyping(Typing event) {
    final typing =
    TypingEvent(from: widget.me.id!, to: widget.receiver.id!, event: event);
